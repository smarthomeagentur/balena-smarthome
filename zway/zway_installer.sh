...
# Fix installation on Raspbian Jessie
IS_JESSIE=`cat /etc/*-release | grep jessie`
if [[ ! -z $IS_JESSIE ]]
then
	echo "Deleting not needed libraries for Raspbian Jessie"
	if [[ -e $TEMP_DIR/z-way-server/libs/libnettle.so ]]
	then
		rm -f $TEMP_DIR/z-way-server/libs/libnettle.so
	fi
	if [[ -e $TEMP_DIR/z-way-server/libs/libnettle.so.4 ]]
	then
		rm -f $TEMP_DIR/z-way-server/libs/libnettle.so.4
	fi
fi
...
if [[ "$?" -eq "0" ]]; then
	...
	if [[ "$upgrade_zway" = "yes" ]]; then
		...
		if [[ -e $TMP_ZWAY_DIR/automation/user_syscommands ]]
			then
			cp $TMP_ZWAY_DIR/automation/user_syscommands $ZWAY_DIR/automation/user_syscommands
		fi
		...
		if [[ -d $TMP_ZWAY_DIR/htdocs/smarthome/user ]]
			then
			rm -Rf $ZWAY_DIR/htdocs/smarthome/user
			cp -R $TMP_ZWAY_DIR/htdocs/smarthome/user $ZWAY_DIR/htdocs/smarthome/
		fi
...
then
	...
	# Add "dtoverlay=pi3-disable-bt" to /boot/config.txt if needed
	if [[ ! `grep "dtoverlay=pi3-disable-bt" /boot/config.txt` ]]
	then
		echo "Adding 'dtoverlay=pi3-disable-bt' to /boot/config.txt"
...
if diff /boot/cmdline.txt /tmp/zway_install_cmdline.txt > /dev/null
then
	rm /tmp/zway_install_cmdline.txt
